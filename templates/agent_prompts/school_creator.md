...
- Do not hardcode `PROXY_SERVER`, `PROXY_USERNAME`, or `PROXY_PASSWORD` in new scripts. Import the shared helper instead:
  `from scrapers.schools.runtime import assert_not_blocklisted, get_playwright_proxy_config, require_proxy_credentials`
- Launch Playwright with `proxy=get_playwright_proxy_config()` so new scrapers inherit the shared mobile proxy configuration automatically.
