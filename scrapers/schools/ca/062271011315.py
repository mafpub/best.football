"""Deterministic athletics availability scraper for Aggeler Community Day (CA)."""
...
NCES_ID = "062271011315"
SCHOOL_NAME = "Aggeler Community Day"
STATE = "CA"
BASE_URL = "https://aggelerchs.lausd.org"
...
MANUAL_PAGES = [
    f"{BASE_URL}/",
    f"{BASE_URL}/apps/pages/",
    f"{BASE_URL}/apps/search/",
    f"{BASE_URL}/apps/events/",
...
async def scrape_school() -> dict[str, Any]:
    """Navigate manually chosen school pages and determine athletics availability."""
    ...
    blocked_page_urls = _dedupe_keep_order(
        [
            ...
            if signal.get("cloudflare_blocked")
        ]
    )
    ...
    if blocked_page_urls and len(blocked_page_urls) == len(source_pages):
        blocked_reason = (
            "Aggeler Community Day public pages loaded only as Cloudflare/Edlio block pages through "
            "the required Oxylabs proxy session, so public athletics content could not be verified."
        )
        errors.append(
        ...
        blocked_reason = (
            "Manual school page navigation did not complete successfully through the required Oxylabs "
            "proxy session, so public athletics content could not be verified."
        )
    else:
        blocked_reason = (
            "Manual school page navigation completed, but no public athletics content was available to "
            "verify on the inspected pages."
        )
