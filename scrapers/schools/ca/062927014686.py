"""Deterministic football scraper for Del Sol High (CA)."""
...
NCES_ID = "062927014686"
SCHOOL_NAME = "Del Sol High"
STATE = "CA"
...
def _extract_school_id(directory_html: str) -> str:
    ...
    for button in soup.select("button.school-btn[data-id]"):
        ...
        if text.lower() == SCHOOL_BUTTON_LABEL.lower():
            school_id = _clean(str(button.get("data-id") or ""))
            if school_id:
                return school_id
    raise RuntimeError("Could not find Del Sol in the CIFSS school directory")
...
async def scrape_school() -> dict[str, Any]:
    """Scrape Del Sol High football signals from the public CIFSS widget."""
    ...
    school_name = _clean(str(school.get("full_name") or school.get("name") or SCHOOL_NAME))
    ...
    extracted_items: dict[str, Any] = {
        ...
        "school_directory_button_label": SCHOOL_BUTTON_LABEL,
        ...
        "football_signals": _dedupe_keep_order(
            [
                f"Directory entry: {SCHOOL_BUTTON_LABEL} (CIFSS school id {school_id})",
                (
